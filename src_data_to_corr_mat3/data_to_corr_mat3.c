...
int main(int argc, char **argv)
{
    ...
    if (rank == MASTER) {
        ...
        int nCat_ua[NCatDim*n_normal];
        for (i=0;i<n_normal;i++) {
            nCat_ua[i*NCatDim+0] = nL_ua[i];
            nCat_ua[i*NCatDim+1] = nN_ua[i];
            nCat_ua[i*NCatDim+2] = nM_ua[i];
            nCat_ua[i*NCatDim+3] = nC_ua[i];
            nCat_ua[i*NCatDim+4] = nV_ua[i];
        }
        double *Ef2cvh;
        Ef2cvh = (double*) malloc(n_normal*sizeof(double));
        obtain_convex_h_qhull(n_normal,Ef2cvh,nCat_ua,Ef_ua,NCatDim,nCat_min,nCat_max,E0,qhullflags);
        ...
        printf("\nStates beyond tol+convex hull were filterd out; n_corr_mat_ug_row: %d\n",n_corr_mat_ug_row);
